...
**Second-order confession**. One way to address performative self-monitoring is to apply the same curriculum recursively. After a model emits a correction, hedge, interruption, or refusal, it can be trained to confess whether that self-monitoring act was genuinely warranted by the evidence or merely performative: a generic safety ritual, an unnecessary hedge, an evasive refusal, or a reward-seeking interruption. These meta-confessions could then be used to train earlier detection of performative monitoring, just as object-level confessions are used to train earlier detection of hallucination or policy violation. The resulting curriculum would scaffold not only interruption of unsafe trajectories, but interruption of bad interruptions.

This raises the obvious regress question — whether meta-confessions require meta-meta-confessions, and so on — but the more plausible hypothesis is that a sufficiently general self-monitoring vocabulary can become level-polymorphic: able to describe object-level failures, monitoring failures, and failures of monitoring-the-monitor without requiring a separately trained channel at each level.
