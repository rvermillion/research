...
Start with a controlled hallucination task against a fixed database. Generate answers under conditions that induce false claims. Train Stage 1 confessions to identify false claims after generation. Then construct Stage 2 and Stage 3 datasets by inserting corrections at controlled positions. Compare four models: baseline, Stage 1 only, Stage 1→2, and Stage 1→2→3. Measure detection accuracy, correction latency, false-positive corrections, and task performance. The key prediction is that staged training reduces correction latency without merely increasing hedging or refusal. The main success criterion would be a leftward shift in correction timing on held-out hallucination cases, without a corresponding rise in false-positive corrections or generic uncertainty language.
