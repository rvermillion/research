...
3. **Differentiable mask and gate:** The predicted retentions generate an additive *forgetful bias* \(B_{t,j}\) applied to attention logits; masked (“student”) outputs \(y^f_t\) are combined with unmasked (“teacher”) outputs \(y_t\) via \(y^{mix}_t=(1-\lambda)y_t+\lambda y^f_t\).  
   The gate λ is annealed from 0→1, smoothly transitioning from teacher supervision to fully on-policy forgetting.
...
For each layer ℓ and token pair (t,j) with \(j\le t\):
...
\]
where τ₍f₎ anneals (sharpens) and γ optionally discounts.  
...
Each layer emits:
\[
y^{mix,(ℓ)} = (1-\lambda_{ℓ})\,y^{(ℓ)} + \lambda_{ℓ}\,y^{f,(ℓ)},
\]
where \(y^{(ℓ)}\) is the full teacher output and \(y^{f,(ℓ)}\) is computed with the forgetful mask.  
As training progresses, λ increases (per schedule or per-layer), gradually shifting reliance from teacher to student.
...
4. Layer-wise λ gating schedules.  
